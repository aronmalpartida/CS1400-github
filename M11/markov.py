...
def generar_texto(modelo, palabra_inicial, longitud):
    ...
    # Paso 5. Bucle para generar palabras hasta alcanzar la longitud deseada
    for _ in range(longitud - 1):
        if palabra_actual in modelo:
            # Elegimos una palabra al azar de la lista de seguidoras
            siguiente = random.choice(modelo[palabra_actual])
            frase_generada.append(siguiente)
            palabra_actual = siguiente
        else:
            # Si la palabra no tiene seguidoras, terminamos antes
            break
